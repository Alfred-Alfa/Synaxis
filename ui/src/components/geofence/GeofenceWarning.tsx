...
export const GeofenceWarning: React.FC<GeofenceWarningProps> = ({ onDismiss, siteName }) => {
    const { countdown, setWarningShown } = useGeofence();
    ...
    useEffect(() => {
        // Auto-hide when countdown reaches 0 (logout will happen)
        if (countdown === 0) {
            setWarningShown(false);
        }
    }, [countdown, setWarningShown]);
        ...
        <div className="fixed inset-0 bg-white/10 backdrop-blur-xl flex items-center justify-center z-[9999] p-4 transition-all animate-in fade-in duration-500">
            <div className="bg-white/90 backdrop-blur-2xl border border-white/40 rounded-[2.5rem] shadow-[0_30px_70px_rgba(0,0,0,0.15)] flex flex-col max-w-sm w-full transform transition-all animate-in zoom-in-95 duration-500 min-h-[500px]">
                <div className="flex-1 flex flex-col items-center text-center p-10">
                    <div className="bg-red-500/15 p-6 rounded-[2rem] mb-10 shadow-inner">
                        <AlertTriangle className="h-12 w-12 text-red-600 animate-pulse" />
                    </div>
                    <h3 className="text-2xl font-black text-gray-900 mb-4 tracking-tight">Location Warning</h3>
                ...
                <div className="px-12 mb-10">
                    <div className="w-full bg-gray-100/50 rounded-full h-3 mb-4 overflow-hidden border border-gray-100/50 p-0.5">
                        <div
                            className="bg-red-500 h-full rounded-full transition-all duration-1000 ease-linear shadow-[0_0_20px_rgba(239,68,68,0.6)]"
                            style={{ width: `${(countdown / 30) * 100}%` }}
                        ></div>
                    </div>
                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400 text-center">
                        Auto-checkout in {countdown}s
                    </p>
                </div>
                ...
                <div className="p-10 pt-0 flex gap-4">
                    <button
                        ...
                        className="flex-1 px-4 py-5 text-sm font-extrabold text-gray-500 hover:bg-gray-100/50 bg-gray-50/50 rounded-2xl transition-all border border-gray-100/50 active:scale-95"
                    >
                        OK
                    </button>
                    ...
                    <button
                        onClick={() => {
                            setWarningShown(false);
                            const onAutoCheckOut = (useGeofence() as any).onAutoCheckOut;
